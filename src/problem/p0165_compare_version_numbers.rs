...
/// being revision 1, and so on. For example 2.5.33 and 0.1 are valid version numbers. To compare
/// version numbers, compare their revisions in left-to-right order. Revisions are compared using
/// their integer value ignoring any leading zeros. This means that revisions 1 and 001 are
...
/// the same, but their revision 1s are 0 and 1 respectively, and 0 < 1. Return the following:
///
    ...
    pub fn compare_version(version1: String, version2: String) -> i32 {
        ...
        for i in 0..max(v1.len(), v2.len()) {
            match v1.get(i).unwrap_or(&0).cmp(v2.get(i).unwrap_or(&0)) {
                Ordering::Equal => (),
                not_equal => return not_equal as i32,
            }
        }

        Ordering::Equal as i32
    }
}
